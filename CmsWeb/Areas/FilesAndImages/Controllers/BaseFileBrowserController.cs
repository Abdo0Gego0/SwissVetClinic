...
namespace CmsWeb.Areas.FilesAndImages.Controllers
{
    ...
    public abstract class BaseFileBrowserController : Controller, IFileBrowserController 
    {
        ...
        /// <exception cref="Exception">Forbidden</exception>
        [AcceptVerbs("POST")]
        public  ActionResult CreateNew(string path1,string path, FileBrowserEntry entry)
        {

            var fullPath = "";
            if (path.IsNullOrEmpty())
            {
                fullPath = Path.Combine(HostingEnvironment.ContentRootPath, "PatientFiles/", path1);
            }
            else
            {
                fullPath = Path.Combine(HostingEnvironment.ContentRootPath, "PatientFiles/", path1,path);
            }
            var name = entry.Name;
        ...
        public ActionResult ReadNew(string path1, string path)
        {
        ...
        protected string NormalizePath(string path)
        {
            ...
            {
                return Path.GetFullPath(Path.Combine(this.HostingEnvironment.ContentRootPath, ContentPath));
            }
            else
            {
                return Path.GetFullPath(Path.Combine(this.HostingEnvironment.ContentRootPath, ContentPath, path));
            }
        }
        ...
        /// <exception cref="Exception">File Not Found</exception>
        [AcceptVerbs("POST")]
        public  ActionResult DestroyNew(string path1, string path, FileBrowserEntry entry)
        {
            var fullPath = "";
            if (path.IsNullOrEmpty())
            {
        ...
        /// <exception cref="Exception">Forbidden</exception>
        [AcceptVerbs("POST")]
        public ActionResult UploadNew(string path1, string path, IFormFile file)
        {
            var fullPath = "";
            if (path.IsNullOrEmpty())
            {
