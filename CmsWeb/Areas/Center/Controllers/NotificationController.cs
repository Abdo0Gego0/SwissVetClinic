...
namespace CmsWeb.Areas.Center.Controllers
{
    ...
    public class NotificationController : Controller
    {
        ...
        public async Task<IActionResult> IndexPolicy(int fromDashboard=0)
        {

            ViewBag.DispalyName = _localizer["Current Policy"];
            ViewBag.PreviousActionDispalyName = _localizer["Notification Policy"];
            ViewBag.PreviousAction = "IndexPolicy";

            return View("CenterAdmin/_NotificationPolicy", medicalCenterService.GetMyNotificationPolicy());
        }

        [HttpPost]
        public async Task<IActionResult> IndexPolicy(NotificationPolicy med)
        {
            ViewBag.DispalyName = _localizer["Current Policy"];
            ViewBag.PreviousActionDispalyName = _localizer["Notification Policy"];
            ViewBag.PreviousAction = "IndexPolicy";

            cmsContext.NotificationPolicy.Attach(med);
            cmsContext.Entry(med).State = EntityState.Modified;
            cmsContext.SaveChanges();
            
            return RedirectToAction("IndexPolicy", "Notification", new {area="CenterAdmin"});
        }
